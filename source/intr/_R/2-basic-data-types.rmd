...
This chapter briefly discusses the basic data types that are used in *R*. We mainly show how to create data of these types. There is much more on how to manipulate data in the following chapters. 
...
To create a vector of several numbers, use the `c` (combine) function:
...
To find elements that fit a particular pattern use `grep` or `grepl`. It returns the index of the matching elements in a vector. You can use the index to subset the original vector (we will see more of this later).
...
Likewise we can test for equality using two equal signs `==` (not `=` which would be an assignment!). `<=` means "smaller or equal" and `>=` means "larger or equal".
