...
Matrices are often created by column-binding and/or row-binding vectors, using the function `cbind` or `rbind`. These are two of the most commonly used functions in *R* so pay close attention!
...
You can also use `cbind` and `rbind` bind (append) matrices, as long as the number of rows or columns of the two objects are the same.
